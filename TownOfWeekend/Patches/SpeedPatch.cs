...
public static class SpeedPatch
{
    ...
    public static void PostfixPhysics(PlayerPhysics __instance)
    {
        if (__instance.AmOwner && GameData.Instance && __instance.myPlayer.CanMove && !__instance.myPlayer.Data.IsDead)
        {
            ...
            foreach (var role in Role.GetRoles(RoleEnum.Venerer))
            {
                ...
                if (venerer.Enabled)
                {
                    if (venerer.KillsAtStartAbility >= 2 && venerer.Player == PlayerControl.LocalPlayer)
                        __instance.body.velocity *= CustomGameOptions.SprintSpeed;
                    else if (venerer.KillsAtStartAbility >= 3)
                        __instance.body.velocity *= CustomGameOptions.FreezeSpeed;
                }
            }
    ...
    public static void PostfixNetwork(CustomNetworkTransform __instance)
    {
        if (!__instance.AmOwner && GameData.Instance && __instance.gameObject.GetComponent<PlayerControl>().CanMove &&
            !__instance.gameObject.GetComponent<PlayerControl>().Data.IsDead)
        {
            ...
            foreach (var role in Role.GetRoles(RoleEnum.Venerer))
            {
                ...
                if (venerer.Enabled)
                {
                    if (venerer.KillsAtStartAbility >= 2 && venerer.Player == player)
                        __instance.body.velocity *= CustomGameOptions.SprintSpeed;
                    else if (venerer.KillsAtStartAbility >= 3)
                        __instance.body.velocity *= CustomGameOptions.FreezeSpeed;
                }
