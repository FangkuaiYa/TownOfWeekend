...
{
    protected internal CustomNumberOption(int id, MultiMenu menu, string name, float value, float min, float max,
        float increment,
        Func<object, string> format = null, bool isHeader = false) : base(id, menu, name, CustomOptionType.Number,
        value, format, isHeader)
    {
        Min = min;
        Max = max;
        Increment = increment;
