...
public static class Patches
{
        ...
        public static void Postfix(GameSettingMenu __instance, int tabNum, bool previewOnly)
        {
            ...
            if (tabNum > 2)
            {
                ...
                if (tabNum >= 5)
                {
                    ...
                    var files = Directory.GetFiles(Application.persistentDataPath, "*.txt")
                        .Select(x => Path.GetFileNameWithoutExtension(x).Split('/')[^1].Split('\\')[^1]).ToList();
                    var num = 1.5f;
                    if (tabNum == 6)
                    {
                        SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, "option.save.new".Translate(),
                            () => SettingsUpdate.ExportSlot(__instance));
                        foreach (var file in files)
                            SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, file,
                                () => SettingsUpdate.ExportSlot(__instance, file));
                    }
                    else
                    {
                        foreach (var file in files)
                            SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, file,
                                () => SettingsUpdate.ImportSlot(__instance, file));
                    }

                    SettingsUpdate.SpawnExternalButton(__instance, tab, ref num, "option.save.return".Translate(),
                        () => Coroutines.Start(TabPatches.ChangeTab(__instance, 3)));
                }
    ...
    private class SettingsUpdate
    {
        ...
        public static void Postfix(GameSettingMenu __instance)
        {
            ...
            CreateSettings(__instance, 3, "ModSettings", "option.button.modSetting", settingsButton, MultiMenu.main);
            CreateSettings(__instance, 4, "CrewSettings", "option.button.crewSetting", settingsButton,
                MultiMenu.crewmate);
            CreateSettings(__instance, 5, "NeutralSettings", "option.button.neuSetting", settingsButton,
                MultiMenu.neutral);
            CreateSettings(__instance, 6, "ImpSettings", "option.button.impSetting", settingsButton,
                MultiMenu.imposter);
            CreateSettings(__instance, 7, "ModifierSettings", "option.button.modifierSt", settingsButton,
                MultiMenu.modifiers);
            CreateSettings(__instance, 8, "ImportSettings", "option.button.importSt", settingsButton,
                MultiMenu.external);
            CreateSettings(__instance, 9, "ExportSettings", "option.button.exprotSt", settingsButton,
                MultiMenu.external);
        }
            ...
            GameObject settingsButton, MultiMenu menu)
        {
            ...
            if (target < 8)
            {
                ...
                if (target == 3)
                {
                    ImportText = SpawnExternalButton(__instance, tabOptions, ref num, "option.button.loadStText",
                        () => Coroutines.Start(TabPatches.ChangeTab(__instance, 8)));
                    ExportText = SpawnExternalButton(__instance, tabOptions, ref num, "option.button.saveStText",
                        () => Coroutines.Start(TabPatches.ChangeTab(__instance, 9)));
                }
        ...
        public static void ExportSlot(GameSettingMenu __instance)
        {
            ...
            var text = Path.Combine(Application.persistentDataPath, $"{"option.preset.saveSt".Translate()} 1.txt");
            var i = 1;

            while (File.Exists(text))
            {
                i++;
                text = Path.Combine(Application.persistentDataPath, $"{"option.preset.saveSt".Translate()} {i}.txt");
            }
    ...
    private class SettingsAwake
    {
        ...
        public static void Postfix(LobbyViewSettingsPane __instance)
        {
            ...
            CreateButton(__instance, 1, "ModTab", "option.button.modSetting", MultiMenu.main, overview);
            CreateButton(__instance, 2, "CrewmateTab", "option.button.crewSetting", MultiMenu.crewmate, overview);
            CreateButton(__instance, 3, "NeutralTab", "option.button.neuSetting", MultiMenu.neutral, overview);
            CreateButton(__instance, 4, "ImpostorTab", "option.button.impSetting", MultiMenu.imposter, overview);
            CreateButton(__instance, 5, "ModifierTab", "option.button.modifierSt", MultiMenu.modifiers, overview);
        }
        ...
        public static void AddSettings(LobbyViewSettingsPane __instance, MultiMenu menu)
        {
            ...
            foreach (var option in options)
                ...
                else
                {
                    if (option.IsHeader)
                    {
                        if (settingsThisHeader % 2 != 0) num -= 0.75f;
                        num -= 0.3f;
                        headingCount += 1;
                        settingsThisHeader = 0;
                    }
