...
public class AssetLoader
{
    ...
    private readonly Dictionary<string, UnityObject> loadedObjects = new();
    private readonly Dictionary<string, string> objectname_to_bundle = new();
    ...
    public void Initialize()
    {
        Array.ForEach(AssetBundles, x =>
        {
            ...
            b.GetAllAssetNames().ToList().ForEach(y => { objectname_to_bundle.Add(ConvertToBaseName(y), x); });
        });
    }
    ...
    private static AssetBundle loadBundle(string bundlename)
    {
        ...
        var stream = assembly.GetManifestResourceStream($"TownOfWeekend.Resources.{bundlename}");
    ...
    private IEnumerator LoadAllAssets(AssetBundle assetBundle)
    {
        ...
        yield return assets;
        foreach (var asset in assets.allAssets) loadedObjects.Add(assetBundle.name, asset.DontUnload());
    }
    ...
    public T Get<T>(string name) where T : UnityObject
    {
        ...
        if (objectname_to_bundle.TryGetValue(name.ToLower(), out var obj2)) return LoadAsset<T>(bundles[obj2], name);
        return null;
    }
}
