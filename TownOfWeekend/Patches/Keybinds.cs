...
using Ability = TownOfWeekend.Roles.Modifiers.Ability;
...
public static class Keybinds
{
    ...
    private static void Prefix(InputManager_Base __instance)
    {
        ...
        __instance.userData.GetAction("ActionSecondary").descriptiveName =
            "Kill / Crew & neutral benign abilities / infect & douse";
        __instance.userData.RegisterBind("ToW imp/nk", "Impostor abilities / ignite");
        __instance.userData.RegisterBind("ToW bb/disperse/mimic", "Button barry / disperse / glitch mimic");
        __instance.userData.RegisterBind("ToW hack", "Glitch's hack");
        __instance.userData.RegisterBind("ToW cycle +", "Cycle forward mimic / transport / guess menu");
        __instance.userData.RegisterBind("ToW cycle -", "Cycle backward mimic / transport / guess menu");
        __instance.userData.RegisterBind("ToW cycle players", "Cycle players as guesser in meetings");
        __instance.userData.RegisterBind("ToW confirm", "Confirm mimic / transport / guess");
    }
...
public sealed class AssassinVigilanteKeybinds
{
    ...
    public static void Postfix(MeetingHud __instance)
    {
        ...
        {
            dynamic guesser = role is Vigilante
                ? Role.GetRole<Vigilante>(role.Player)
                : Ability.GetAbility<Assassin>(role.Player);
            if (guesser == null) guesser = Role.GetRole<Doomsayer>(role.Player);
            var players = __instance.playerStates.Where(x =>
                    (guesser as IGuesser).Buttons[x.TargetPlayerId] != (null, null, null, null)
                    && x.TargetPlayerId != role.Player.PlayerId)
                .ToList();

            if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle players"))
            {
            ...
            if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle +"))
                (guesser as IGuesser).Buttons[HighlightedPlayer.TargetPlayerId].Item2.GetComponent<PassiveButton>()
                    .OnClick.Invoke();
            else if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle -"))
                (guesser as IGuesser).Buttons[HighlightedPlayer.TargetPlayerId].Item1.GetComponent<PassiveButton>()
                    .OnClick.Invoke();
            else if (ReInput.players.GetPlayer(0).GetButtonDown("ToW confirm"))
                (guesser as IGuesser).Buttons[HighlightedPlayer.TargetPlayerId].Item3.GetComponent<PassiveButton>()
                    .OnClick.Invoke();
        }
    }
