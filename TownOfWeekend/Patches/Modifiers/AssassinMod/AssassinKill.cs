...
using TownOfWeekend.ImpostorRoles.BlackmailerMod;
...
public class AssassinKill
{
    ...
    )
    {
        ...
        if (amOwner) meetingHud.SetForegroundForDead();

        var blackmailers = Role.AllRoles.Where(x => x.RoleType == RoleEnum.Blackmailer && x.Player != null)
            .Cast<Blackmailer>();
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Swapper) && !PlayerControl.LocalPlayer.Data.IsDead)
        {
            ...
            if (button.GetComponent<SpriteRenderer>().sprite == ResourcesManager.SwapperSwitch)
            {
