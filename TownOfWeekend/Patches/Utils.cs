...
using TownOfWeekend.CrewmateRoles.AurialMod;
...
using TownOfWeekend.CrewmateRoles.VampireHunterMod;
...
using PerformKill = TownOfWeekend.Modifiers.UnderdogMod.PerformKill;
...
public static class Utils
{
    ...
    private static NetworkedPlayerInfo voteTarget;
    ...
    public static void Morph(PlayerControl player, PlayerControl MorphedPlayer, bool resetAnim = false)
    {
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Aurial) &&
            !Role.GetRole<Aurial>(PlayerControl.LocalPlayer).NormalVision) return;
    ...
    {
        if (!(PlayerControl.LocalPlayer.Is(RoleEnum.Aurial) &&
              !Role.GetRole<Aurial>(PlayerControl.LocalPlayer).NormalVision))
    ...
    public static System.Collections.Generic.List<PlayerControl> GetCrewmates(
        System.Collections.Generic.List<PlayerControl> impostors)
    {
    ...
    public static System.Collections.Generic.List<PlayerControl> GetImpostors(
        System.Collections.Generic.List<NetworkedPlayerInfo> infected)
    {
        var impostors = new System.Collections.Generic.List<PlayerControl>();
    ...
    public static bool IsInfected(this PlayerControl player)
    {
        return Role.GetRoles(RoleEnum.Plaguebearer).Any(role =>
        {
            ...
            return plaguebearer != null && (plaguebearer.InfectedPlayers.Contains(player.PlayerId) ||
                                            player.PlayerId == plaguebearer.Player.PlayerId);
        });
    }

    public static System.Collections.Generic.List<bool> Interact(PlayerControl player, PlayerControl target,
        bool toKill = false)
    {
        var fullCooldownReset = false;
        var gaReset = false;
        var survReset = false;
        var zeroSecReset = false;
        var abilityUsed = false;
        if (target.IsInfected() || player.IsInfected())
            foreach (var pb in Role.GetRoles(RoleEnum.Plaguebearer))
                ((Plaguebearer)pb).RpcSpreadInfection(target, player);
        ...
        var reset = new System.Collections.Generic.List<bool>();
    ...
    public static List<PlayerControl> GetClosestPlayers(Vector2 truePosition,
        float radius, bool includeDead)
    {
        var playerControlList = new List<PlayerControl>();
        ...
        for (var index = 0; index < allPlayers.Count; ++index)
        {
            ...
            {
                var vector2 = new Vector2(playerInfo.Object.GetTruePosition().x - truePosition.x,
                    playerInfo.Object.GetTruePosition().y - truePosition.y);
    ...
    public static PlayerControl GetClosestPlayer(PlayerControl refPlayer,
        System.Collections.Generic.List<PlayerControl> AllPlayers)
    {
    ...
    public static void SetTarget(
        ...
        KillButton button,
        float maxDistance = float.NaN, System.Collections.Generic.List<PlayerControl> targets = null
    )
    {
    ...
    public static void MurderPlayer(PlayerControl killer, PlayerControl target, bool jumpToBody)
    {
        ...
        if (data != null && !data.IsDead)
        {
            ...
            if (!killer.Is(Faction.Crewmates) && killer != target
                                              ...
                                              GameModes.Normal) Role.GetRole(killer).Kills += 1;
            ...
            if (killer.Is(RoleEnum.Veteran))
            {
                ...
                if (target.Is(Faction.Impostors) || target.Is(Faction.NeutralKilling) || target.Is(Faction.NeutralEvil))
                    veteran.CorrectKills += 1;
            ...
            if (PlayerControl.LocalPlayer.Is(RoleEnum.Mystic) && !PlayerControl.LocalPlayer.Data.IsDead)
                Coroutines.Start(FlashCoroutine(Colors.Mystic));
            ...
            if (target.Is(ModifierEnum.Bait)) BaitReport(killer, target);

            if (target.Is(ModifierEnum.Aftermath)) Aftermath.ForceAbility(killer, target);
            ...
            if (target.Is(ModifierEnum.Diseased) && killer.Is(RoleEnum.Werewolf))
            {
                ...
                werewolf.LastKilled =
                    DateTime.UtcNow.AddSeconds((CustomGameOptions.DiseasedMultiplier - 1f) *
                                               CustomGameOptions.RampageKillCd);
            ...
            if (target.Is(ModifierEnum.Diseased) && killer.Is(RoleEnum.Vampire))
            {
                ...
                vampire.LastBit =
                    DateTime.UtcNow.AddSeconds((CustomGameOptions.DiseasedMultiplier - 1f) * CustomGameOptions.BiteCd);
            ...
            if (target.Is(ModifierEnum.Diseased) && killer.Is(RoleEnum.Glitch))
            {
                ...
                glitch.LastKill = DateTime.UtcNow.AddSeconds((CustomGameOptions.DiseasedMultiplier - 1f) *
            ...
            if (target.Is(ModifierEnum.Diseased) && killer.Is(RoleEnum.Juggernaut))
            {
                ...
                juggernaut.LastKill = DateTime.UtcNow.AddSeconds((CustomGameOptions.DiseasedMultiplier - 1f) *
                                                                 (CustomGameOptions.JuggKCd -
                                                                  CustomGameOptions.ReducedKCdPerKill *
                                                                  juggernaut.JuggKills));
                juggernaut.Player.SetKillTimer(
                    (CustomGameOptions.JuggKCd - CustomGameOptions.ReducedKCdPerKill * juggernaut.JuggKills) *
            ...
            if (target.Is(ModifierEnum.Diseased) && killer.Is(ModifierEnum.Underdog))
            {
                var lowerKC =
                    (GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown -
                     CustomGameOptions.UnderdogKillBonus) * CustomGameOptions.DiseasedMultiplier;
            ...
            {
                var lowerKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown -
                              CustomGameOptions.UnderdogKillBonus;
                var normalKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown;
                var upperKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown +
                              CustomGameOptions.UnderdogKillBonus;
                killer.SetKillTimer(PerformKill.LastImp() ? lowerKC : PerformKill.IncreasedKC() ? normalKC : upperKC);
                return;
            }
    ...
    {
        var extraDelay = Random.RandomRangeInt(0,
            (int)(100 * (CustomGameOptions.BaitMaxDelay - CustomGameOptions.BaitMinDelay) + 1));
    ...
    {
        if (PlayerControl.LocalPlayer == player) Coroutines.Start(FlashCoroutine(Colors.Impostor));
        if (PlayerControl.LocalPlayer != player && PlayerControl.LocalPlayer.Is(RoleEnum.CultistMystic)
                                                && !PlayerControl.LocalPlayer.Data.IsDead)
            Coroutines.Start(FlashCoroutine(Colors.Impostor));
        ...
        if (player.Is(RoleEnum.CultistSnitch))
        {
            ...
            snitch.Color = Colors.Impostor;
            snitch.Faction = Faction.Impostors;
            snitch.RegenTask();
            if (PlayerControl.LocalPlayer == player && snitch.CompletedTasks)
            {
                var crew = PlayerControl.AllPlayerControls.ToArray()
                    .Where(x => x.Is(Faction.Crewmates) && !x.Is(RoleEnum.Mayor)).ToList();
                if (crew.Count != 0)
                {
                    crew.Shuffle();
        ...
        if (player.Is(RoleEnum.Vigilante))
        {
            ...
            vigi.RegenTask();
            var colorMapping = new System.Collections.Generic.Dictionary<string, Color>();
    ...
    public static System.Collections.Generic.IEnumerable<(T1, T2)> Zip<T1, T2>(
        System.Collections.Generic.List<T1> first, System.Collections.Generic.List<T2> second)
    {
    ...
    {
        var totalTasks = GameOptionsManager.Instance.currentNormalGameOptions.NumCommonTasks +
        ...
        foreach (var task in player.myTasks)
            if (task.TryCast<NormalPlayerTask>() != null)
            {
                ...
                if ((normalPlayerTask.TaskType == TaskTypes.EmptyGarbage ||
                     normalPlayerTask.TaskType == TaskTypes.EmptyChute)
    ...
    public static void DestroyAll(this System.Collections.Generic.IEnumerable<Component> listie)
    {
    ...
    public static void Rpc(params object[] data)
    {
        if (data[0] is not CustomRPC)
            throw new ArgumentException($"first parameter must be a {typeof(CustomRPC).FullName}");
        ...
        foreach (var item in data[1..])
            if (item is bool boolean)
                ...
                Logger<TownOfWeekendPlugin>.Error(
                    $"unknown data type entered for rpc write: item - {nameof(item)}, {item.GetType().FullName}, rpc - {data[0]}");
    ...
    {
        var genericListType = typeof(System.Collections.Generic.List<>).MakeGenericType(myType);
    ...
    public static void ResetCustomTimers()
    {
        ...
        foreach (var vh in Role.GetRoles(RoleEnum.VampireHunter))
        {
            ...
            var vamps = PlayerControl.AllPlayerControls.ToArray()
                .Where(x => x.Is(RoleEnum.Vampire) && !x.Data.IsDead && !x.Data.Disconnected).ToList();
            if (vamps.Count == 0 && vh.Player != StartImitate.ImitatingPlayer && !vh.Player.Data.IsDead &&
                !vh.Player.Data.Disconnected)
            {
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Escapist))
        {
            ...
            escapist.LastEscape = DateTime.UtcNow;
            escapist.EscapeButton.graphic.sprite = ResourcesManager.MarkSprite;
        }
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Bomber))
        {
            ...
            bomber.PlantButton.graphic.sprite = ResourcesManager.PlantSprite;
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Morphling))
        {
            ...
            morphling.MorphButton.graphic.sprite = ResourcesManager.SampleSprite;
        ...
        if (PlayerControl.LocalPlayer.Is(RoleEnum.Undertaker))
        {
            ...
            undertaker.DragDropButton.graphic.sprite = ResourcesManager.DragSprite;
    ...
    private class MedScanMinigameFixedUpdatePatch
    {
