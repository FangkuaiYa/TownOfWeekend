...
public static class NEWin
{
    public static void Postfix(EndGameManager __instance)
    {
        ...
        var doomRole = Role.AllRoles.FirstOrDefault(x =>
            ...
            ((Doomsayer)x).Player == PlayerControl.LocalPlayer);
        ...
        var exeRole = Role.AllRoles.FirstOrDefault(x =>
            ...
            ((Executioner)x).Player == PlayerControl.LocalPlayer);
        if (exeRole != null) neWin = true;
        var jestRole = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Jester && ((Jester)x).VotedOut && ((Jester)x).Player == PlayerControl.LocalPlayer);
        if (jestRole != null) neWin = true;
        var phantomRole = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Phantom && ((Phantom)x).CompletedTasks &&
            ((Phantom)x).Player == PlayerControl.LocalPlayer);
        if (phantomRole != null) neWin = true;
        if (neWin)
        {
            __instance.WinText.text = "</color><color=#008DFFFF>Victory";
            var loveRole =
                Modifier.AllModifiers.FirstOrDefault(x =>
                    x.ModifierType == ModifierEnum.Lover && ((Lover)x).LoveCoupleWins);
            ...
            var juggRole =
                Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Juggernaut && ((Juggernaut)x).JuggernautWins);
            if (juggRole != null) return;
            var pestRole =
                Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Pestilence && ((Pestilence)x).PestilenceWins);
            if (pestRole != null) return;
            var pbRole = Role.AllRoles.FirstOrDefault(x =>
                x.RoleType == RoleEnum.Plaguebearer && ((Plaguebearer)x).PlaguebearerWins);
            if (pbRole != null) return;
            var wwRole =
                Role.AllRoles.FirstOrDefault(x => x.RoleType == RoleEnum.Werewolf && ((Werewolf)x).WerewolfWins);
