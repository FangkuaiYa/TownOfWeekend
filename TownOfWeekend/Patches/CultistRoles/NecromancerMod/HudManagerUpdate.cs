...
namespace TownOfWeekend.CultistRoles.NecromancerMod;

[HarmonyPatch(typeof(HudManager), nameof(HudManager.Update))]
public class ReviveHudManagerUpdate
{
    public static byte DontRevive = byte.MaxValue;
    public static Sprite ReviveSprite => ResourcesManager.Revive2Sprite;
    ...
    public static void Postfix(HudManager __instance)
    {
        ...
        role.ReviveButton.gameObject.SetActive(
        ...
        if (role.CurrentTarget && role.CurrentTarget != closestBody)
            foreach (var body in role.CurrentTarget.bodyRenderers)
                body.material.SetFloat("_Outline", 0f);
        ...
        if (role.CurrentTarget && role.ReviveButton.enabled &&
            !(player.Is(RoleEnum.Sheriff) || player.Is(RoleEnum.CultistSeer) || player.Is(RoleEnum.Survivor) ||
              player.Is(RoleEnum.Mayor)) &&
            !(PlayerControl.LocalPlayer.killTimer >
              GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown - 0.5f))
        {
