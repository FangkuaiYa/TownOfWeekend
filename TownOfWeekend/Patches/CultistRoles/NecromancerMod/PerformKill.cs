...
public class PerformRevive
{
    public static bool Prefix(KillButton __instance)
    {
        ...
        if (__instance == role.ReviveButton)
        {
            ...
            if (player.Is(RoleEnum.Sheriff) || player.Is(RoleEnum.CultistSeer) || player.Is(RoleEnum.Survivor) ||
                player.Is(RoleEnum.Mayor)) return false;
            if (PlayerControl.LocalPlayer.killTimer >
                GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown - 0.5f) return false;
