...
namespace TownOfWeekend.CultistRoles.SnitchMod;
    ...
    public static void Postfix(PlayerControl __instance)
    {
        ...
        if (tasksLeft == 0)
        {
            ...
            if (PlayerControl.LocalPlayer == role.Player)
            {
                ...
                {
                    var imps = PlayerControl.AllPlayerControls.ToArray().Where(x =>
                        x.Is(Faction.Impostors) && !x.Is(RoleEnum.Necromancer) && !x.Is(RoleEnum.Whisperer)).ToList();
                    if (imps.Count == 0)
                        imps = PlayerControl.AllPlayerControls.ToArray()
