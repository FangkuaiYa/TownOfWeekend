...
namespace TownOfWeekend.CultistRoles.WhispererMod;
...
public class ColorNameUpdate
{
    private static void UpdateMeeting(MeetingHud __instance)
    {
        ...
        foreach (var state in __instance.playerStates)
        {
            ...
            var color = stats.Item2 / 100f;
            if (color <= 0) state.NameText.color = Colors.Impostor;
    ...
    private static void Postfix(HudManager __instance)
    {
        ...
        if (!(PlayerControl.LocalPlayer.Is(RoleEnum.Whisperer) ||
              PlayerControl.LocalPlayer.Is(RoleEnum.Necromancer))) return;
        ...
        {
            var color = stats.Item2 / 100f;
            if (color <= 0) stats.Item1.nameText().color = Colors.Impostor;
