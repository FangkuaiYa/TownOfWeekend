...
{
    public static Sprite MimicSprite = ResourcesManager.MimicSprite;
    public static Sprite HackSprite = ResourcesManager.HackSprite;
    public static Sprite LockSprite = ResourcesManager.LockSprite;
    ...
    {
        if (!Player.Data.IsDead) Utils.SetClosestPlayer(ref ClosestPlayer);
    ...
    public void FixedUpdate(HudManager __instance)
    {
        ...
        if (MimicList != null)
        {
            ...
            {
                if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle +"))
                {
                    PlayerIndex = PlayerIndex == MimicList.chatBubblePool.activeChildren.Count - 1
                        ? 0
                        : PlayerIndex + 1;
                    HighlightedPlayer = MimicList.chatBubblePool.activeChildren[PlayerIndex];
                }
                else if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle -"))
                {
                    PlayerIndex = PlayerIndex == 0
                        ? MimicList.chatBubblePool.activeChildren.Count - 1
                        : PlayerIndex - 1;
                    HighlightedPlayer = MimicList.chatBubblePool.activeChildren[PlayerIndex];
                }
                else if (ReInput.players.GetPlayer(0).GetButtonDown("ToW confirm") && HighlightedPlayer)
                {
    ...
    public static class AbilityCoroutine
    {
        ...
        public static IEnumerator Hack(Glitch __instance, PlayerControl hackPlayer)
        {
            ...
            while (true)
            {
                ...
                if (MeetingHud.Instance || totalHacktime > CustomGameOptions.HackDuration ||
                    hackPlayer?.Data.IsDead != false)
                {
                    foreach (var obj in lockImg) obj?.SetActive(false);
        ...
        public static IEnumerator Mimic(Glitch __instance, PlayerControl mimicPlayer)
        {
            ...
            while (true)
            {
                ...
                if (__instance.Player.Data.IsDead) totalMimickTime = CustomGameOptions.MimicDuration;
                mimicText.Text =
    ...
    public static class KillButtonHandler
    {
        ...
        {
            if (!__gInstance.Player.Data.IsImpostor() && ReInput.players.GetPlayer(0).GetButtonDown(8))
                __instance.KillButton.DoClick();

            __instance.KillButton.gameObject.SetActive(
    ...
    public static class HackButtonHandler
    {
        public static void HackButtonUpdate(Glitch __gInstance, HudManager __instance)
        {
            if (__gInstance.HackButton == null)
            {
                __gInstance.HackButton =
                    Object.Instantiate(__instance.KillButton, __instance.KillButton.transform.parent);
            ...
            __gInstance.HackButton.gameObject.SetActive(
                (__instance.UseButton.isActiveAndEnabled || __instance.PetButton.isActiveAndEnabled)
                && !MeetingHud.Instance && !__gInstance.Player.Data.IsDead
                && AmongUsClient.Instance.GameState == InnerNetClient.GameStates.Started);
            ...
            if (__gInstance.HackButton.isActiveAndEnabled)
            {
                ...
                Utils.SetTarget(
                    ref closestPlayer,
                    __gInstance.HackButton,
                    LegacyGameOptions.KillDistances[CustomGameOptions.GlitchHackDistance]
                );
            ...
            if (__gInstance.HackTarget != null)
            {
                ...
                if (ReInput.players.GetPlayer(0).GetButtonDown("ToW hack")) __gInstance.HackButton.DoClick();
            }
        }
        ...
        public static void HackButtonPress(Glitch __gInstance)
        {
            ...
            if (__gInstance.HackTarget != null)
            {
                ...
                if (interact[4]) __gInstance.RpcSetHacked(__gInstance.HackTarget);
                if (interact[0])
                {
    ...
    public static class MimicButtonHandler
    {
        public static void MimicButtonUpdate(Glitch __gInstance, HudManager __instance)
        {
            ...
            __gInstance.MimicButton.gameObject.SetActive(
                (__instance.UseButton.isActiveAndEnabled || __instance.PetButton.isActiveAndEnabled)
                && !MeetingHud.Instance && !__gInstance.Player.Data.IsDead
                && AmongUsClient.Instance.GameState == InnerNetClient.GameStates.Started);
            ...
            if (!__gInstance.MimicButton.isCoolingDown && !__gInstance.IsUsingMimic)
            {
                ...
                if (ReInput.players.GetPlayer(0).GetButtonDown("ToW bb/disperse/mimic"))
        ...
        public static void MimicButtonPress(Glitch __gInstance)
        {
            if (__gInstance.MimicList == null)
            {
                ...
                __gInstance.MimicList.openKeyboardButton.transform.GetChild(0).gameObject.GetComponent<SpriteRenderer>()
                    .enabled = false;
