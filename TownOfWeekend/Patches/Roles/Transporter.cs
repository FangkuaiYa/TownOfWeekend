...
public class Transporter : Role
{
    ...
    public void FixedUpdate(HudManager __instance)
    {
        if (PressedButton && TransportList == null)
        {
            ...
            TransportList.openKeyboardButton.transform.GetChild(0).gameObject.GetComponent<SpriteRenderer>().enabled =
                false;
                    ...
                    TempPlayer.PlayerId != PlayerControl.LocalPlayer.PlayerId)
                {
                    foreach (var player in PlayerControl.AllPlayerControls)
                             ...
                             Object.FindObjectsOfType<DeadBody>().Any(x => x.ParentId == player.PlayerId)))
                        {
                            ...
                            TransportList.chatBubblePool
                                .activeChildren[TransportList.chatBubblePool.activeChildren._size - 1]
                                .Cast<ChatBubble>().SetName(player.Data.PlayerName, false, false,
                                    PlayerControl.LocalPlayer.PlayerId == player.PlayerId ? Color : Color.white);
                            var IsDeadTemp = player.Data.IsDead;
                            player.Data.IsDead = false;
                            TransportList.chatBubblePool
                                .activeChildren[TransportList.chatBubblePool.activeChildren._size - 1]
                                .Cast<ChatBubble>().SetCosmetics(player.Data);
        ...
        if (TransportList != null)
        {
            ...
            if (!TransportList.IsOpenOrOpening || MeetingHud.Instance || Input.GetKeyInt(KeyCode.Escape) ||
            ...
            else
            {
                ...
                {
                    PlayerIndex = PlayerIndex == TransportList.chatBubblePool.activeChildren.Count - 1
                        ? 0
                        : PlayerIndex + 1;
                ...
                else if (ReInput.players.GetPlayer(0).GetButtonDown("ToW cycle -"))
                {
                    PlayerIndex = PlayerIndex == 0
                        ? TransportList.chatBubblePool.activeChildren.Count - 1
                        : PlayerIndex - 1;
                    HighlightedPlayer = TransportList.chatBubblePool.activeChildren[PlayerIndex];
                }
    ...
    public static IEnumerator TransportPlayers(byte player1, byte player2, bool die)
    {
        ...
        {
            while (SubmergedCompatibility.getInTransition()) yield return null;
        ...
        if (Player1Body == null && Player2Body == null)
        {
            ...
            if (die)
            {
                Utils.MurderPlayer(TP1, TP2, true);
            }
            else
            {
            ...
            !UntransportablePlayers.ContainsKey(TransportPlayer2.PlayerId))
        {
            ...
            if (TransportPlayer1.Is(RoleEnum.Pestilence) || TransportPlayer1.IsOnAlert())
            {
                if (Player.IsShielded())
                {
                    ...
                    StopKill.BreakShield(Player.GetMedic().Player.PlayerId, Player.PlayerId,
            ...
            if (TransportPlayer2.Is(RoleEnum.Pestilence) || TransportPlayer2.IsOnAlert())
            {
                if (Player.IsShielded())
                {
                    Utils.Rpc(CustomRPC.AttemptSound, Player.GetMedic().Player.PlayerId, Player.PlayerId);
