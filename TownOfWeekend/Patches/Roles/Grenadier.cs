...
public class Grenadier : Role
{
    ...
    private static readonly Color normalVision = new(0.6f, 0.6f, 0.6f, 0f);
    private static readonly Color dimVision = new(0.6f, 0.6f, 0.6f, 0.2f);
    private static readonly Color blindVision = new(0.6f, 0.6f, 0.6f, 1f);
            ...
            if (PlayerControl.LocalPlayer.PlayerId == player.PlayerId)
            {
                if (TimeRemaining > CustomGameOptions.GrenadeDuration - 0.5f && !sabActive)
                {
                    ...
                    else if (ShouldPlayerBeDimmed(player))
                    {
                        ...
                        HudManager.Instance.FullScreen.gameObject.active = true;
                        HudManager.Instance.FullScreen.color =
                            Color.Lerp(normalVision, dimVision, fade);
                ...
                else if (TimeRemaining < 0.5f && !sabActive)
                {
                    ...
                    if (ShouldPlayerBeBlinded(player))
                    {
                        ...
                        HudManager.Instance.FullScreen.gameObject.active = true;
                        HudManager.Instance.FullScreen.color =
                            Color.Lerp(blindVision, normalVision, fade2);
                    }
                    else if (ShouldPlayerBeDimmed(player))
                    {
                        HudManager.Instance.FullScreen.enabled = true;
                        HudManager.Instance.FullScreen.gameObject.active = true;
                        HudManager.Instance.FullScreen.color =
                            Color.Lerp(dimVision, normalVision, fade2);
                        if (PlayerControl.LocalPlayer.Data.IsImpostor() &&
                            MapBehaviour.Instance.infectedOverlay.sabSystem.Timer < 0.5f)
                            MapBehaviour.Instance.infectedOverlay.sabSystem.Timer = 0.5f;
                    }
