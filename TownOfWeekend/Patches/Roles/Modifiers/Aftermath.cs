...
using TownOfWeekend.ImpostorRoles.UndertakerMod;
using TownOfWeekend.Modifiers.UnderdogMod;
...
public class Aftermath : Modifier
{
    ...
    public static void ForceAbility(PlayerControl player, PlayerControl corpse)
    {
        ...
        foreach (var body in bodies)
            try
            {
                if (body?.ParentId == corpse.PlayerId)
                {
                    db = body;
                    break;
                }
    ...
    private static IEnumerator delay(PlayerControl player, PlayerControl corpse, DeadBody db)
    {
        ...
        else if (role is Janitor janitor)
        {
            ...
            Coroutines.Start(Coroutine.CleanCoroutine(db, janitor));
        }
        ...
        else if (role is Morphling morphling)
        {
            if (morphling.Player.GetCustomOutfitType() != CustomPlayerOutfitType.Morph)
            {
                ...
                if (morphling.SampledPlayer == null)
                    morphling._morphButton.graphic.sprite = ResourcesManager.MorphSprite;
        ...
        else if (role is Undertaker undertaker)
        {
            ...
            KillButtonTarget.SetTarget(undertaker._dragDropButton, null, undertaker);
            undertaker._dragDropButton.graphic.sprite = ResourcesManager.DropSprite;
        }
        ...
        else if (role is Bomber bomber)
        {
            ...
            bomber.PlantButton.graphic.sprite = ResourcesManager.DetonateSprite;
            ...
            if (PlayerControl.LocalPlayer.Is(ModifierEnum.Underdog))
            {
                var lowerKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown -
                    CustomGameOptions.UnderdogKillBonus + CustomGameOptions.DetonateDelay;
                var normalKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown +
                               CustomGameOptions.DetonateDelay;
                var upperKC = GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown +
                              CustomGameOptions.UnderdogKillBonus + CustomGameOptions.DetonateDelay;
                PlayerControl.LocalPlayer.SetKillTimer(PerformKill.LastImp() ? lowerKC :
                    PerformKill.IncreasedKC() ? normalKC : upperKC);
            }
            else
            {
