...
public class Lover : Modifier
{
    ...
    public static void Gen(List<PlayerControl> canHaveModifiers)
    {
        ...
        foreach (var player in canHaveModifiers)
            if (player.Is(Faction.Impostors) || (player.Is(Faction.NeutralKilling) && !player.Is(RoleEnum.Vampire) &&
                                                 CustomGameOptions.NeutralLovers))
                impostors.Add(player);
            else if (player.Is(Faction.Crewmates) || (player.Is(Faction.NeutralBenign) &&
                                                      CustomGameOptions.NeutralLovers)
        ...
        else
        {
            ...
            while (num3 == num) num3 = Random.RandomRangeInt(0, crewmates.Count);
    ...
    private bool FourPeopleLeft()
    {
        ...
        {
            return !lover1.Data.IsDead && !lover1.Data.Disconnected && !lover2.Data.IsDead &&
                   !lover2.Data.Disconnected &&
    ...
    public void Win()
    {
        if (CustomGameOptions.NeutralEvilWinEndsGame &&
            Role.AllRoles.Where(x => x.RoleType == RoleEnum.Jester).Any(x => ((Jester)x).VotedOut)) return;
        LoveCoupleWins = true;
