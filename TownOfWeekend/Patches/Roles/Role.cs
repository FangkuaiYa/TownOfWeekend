...
public abstract class Role
{
    ...
    public List<KillButton> ExtraButtons = new();
    ...
    {
        return DeadCriteria() || ImpostorCriteria() || VampireCriteria() || LoverCriteria() || SelfCriteria() ||
               RoleCriteria() || GuardianAngelCriteria() || Local;
    }

    internal virtual bool ColorCriteria()
    {
        return SelfCriteria() || DeadCriteria() || ImpostorCriteria() || VampireCriteria() || RoleCriteria() ||
               GuardianAngelCriteria();
    }
    ...
    {
        return PlayerControl.LocalPlayer.Is(ModifierEnum.Sleuth) &&
               Modifier.GetModifier<Sleuth>(PlayerControl.LocalPlayer).Reported.Contains(Player.PlayerId);
    }
    ...
    internal virtual bool GuardianAngelCriteria()
    {
        return PlayerControl.LocalPlayer.Is(RoleEnum.GuardianAngel) && CustomGameOptions.GAKnowsTargetRole &&
               Player == GetRole<GuardianAngel>(PlayerControl.LocalPlayer).target;
    }
    ...
    protected virtual string NameText(bool revealTasks, bool revealRole, bool revealModifier, bool revealLover,
        PlayerVoteArea player = null)
    {
        if (CamouflageUnCamouflage.IsCamoed && player == null) return "";

        if (Player == null) return "";

        var PlayerName = Player.GetDefaultOutfit().PlayerName;
        ...
        if (revealTasks && (Faction == Faction.Crewmates || RoleType == RoleEnum.Phantom))
            if ((PlayerControl.LocalPlayer.Data.IsDead && CustomGameOptions.SeeTasksWhenDead) ||
                (MeetingHud.Instance && CustomGameOptions.SeeTasksDuringMeeting) ||
                (!PlayerControl.LocalPlayer.Data.IsDead && !MeetingHud.Instance &&
                 CustomGameOptions.SeeTasksDuringRound))
    ...
    public static T GenRole<T>(Type type, PlayerControl player)
    {
        ...
        Utils.Rpc(CustomRPC.SetRole, player.PlayerId, type.FullName);
        return role;
    }
    ...
    public static T GenModifier<T>(Type type, PlayerControl player)
    {
        ...
        Utils.Rpc(CustomRPC.SetModifier, player.PlayerId, type.FullName);
        return modifier;
    }
    ...
    public static class IntroCutScenePatch
    {
        ...
        public static class IntroCutscene_ShowTeam__d_MoveNext
        {
            ...
            public static void Postfix(IntroCutscene._ShowRole_d__41 __instance)
            {
                ...
                {
                    if (role.Faction == Faction.NeutralKilling || role.Faction == Faction.NeutralEvil ||
                        role.Faction == Faction.NeutralBenign)
                    {
            ...
            public static void Postfix(IntroCutscene._ShowRole_d__41 __instance)
            {
                ...
                if (CustomGameOptions.GameMode == GameMode.AllAny && CustomGameOptions.RandomNumberImps)
                    __instance.__4__this.ImpostorText.text =
                        "There are an <color=#FF0000FF>Unknown Number of Impostors</color> among us";
            }
        }
    ...
    public static class ShipStatus_KMPKPPGPNIH
    {
        ...
        public static bool Prefix(LogicGameFlowNormal __instance)
        {
            ...
            if (ShipStatus.Instance.Systems != null)
            {
                ...
                {
                    var lifeSuppSystemType =
                        ShipStatus.Instance.Systems[SystemTypes.LifeSupp].Cast<LifeSuppSystemType>();
                    if (lifeSuppSystemType.Countdown < 0f) return true;
                }

                if (ShipStatus.Instance.Systems.ContainsKey(SystemTypes.Laboratory))
                {
                    var reactorSystemType =
                        ShipStatus.Instance.Systems[SystemTypes.Laboratory].Cast<ReactorSystemType>();
            ...
            foreach (var role in AllRoles)
            {
                ...
                var modifierIsEnd = true;
                var alives = PlayerControl.AllPlayerControls.ToArray()
                    .Where(x => !x.Data.IsDead && !x.Data.Disconnected).ToList();
                var impsAlive = PlayerControl.AllPlayerControls.ToArray()
                    .Where(x => !x.Data.IsDead && !x.Data.Disconnected && x.Data.IsImpostor()).ToList();
                    ...
                    if (ExileController.Instance.initData.networkedPlayer == null)
                        foreach (var oracle in GetRoles(RoleEnum.Oracle))
                        {
                            ...
                            if (oracleRole.SavedConfessor)
                            {
                                ...
                                __result =
                                    $"{oracleRole.Confessor.GetDefaultOutfit().PlayerName} was blessed by an Oracle!";
                            }
                        }
    ...
    public static class HudManager_Update
    {
        ...
        private static void UpdateMeeting(MeetingHud __instance)
        {
            foreach (var player in __instance.playerStates)
            {
                ...
                {
                    var selfFlag = role.SelfCriteria();
                    var deadFlag = role.DeadCriteria();
                    var impostorFlag = role.ImpostorCriteria();
                    var vampireFlag = role.VampireCriteria();
                    var loverFlag = role.LoverCriteria();
                    var roleFlag = role.RoleCriteria();
                    var gaFlag = role.GuardianAngelCriteria();
        ...
        private static void Postfix(HudManager __instance)
        {
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                ...
                if (player.Data != null && PlayerControl.LocalPlayer.Data.IsImpostor() &&
                    player.Data.IsImpostor()) continue;
            }
        }
    }
}