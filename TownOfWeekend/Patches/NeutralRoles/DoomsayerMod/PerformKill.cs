...
public class PerformKill
{
    public static bool Prefix(KillButton __instance)
    {
        ...
        if (interact[4]) role.LastObservedPlayer = role.ClosestPlayer;
        if (interact[0])
        {
        ...
        if (interact[1])
        {
            role.LastObserved = DateTime.UtcNow;
            role.LastObserved =
                role.LastObserved.AddSeconds(CustomGameOptions.ProtectKCReset - CustomGameOptions.ObserveCooldown);
            return false;
        }
