...
namespace TownOfWeekend.NeutralRoles.DoomsayerMod;
...
public class MeetingStart
{
    public static void Postfix(MeetingHud __instance)
    {
        ...
        if (doomsayerRole.LastObservedPlayer != null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(playerResults))
                HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, playerResults);
            if (!string.IsNullOrWhiteSpace(roleResults))
                HudManager.Instance.Chat.AddChat(PlayerControl.LocalPlayer, roleResults);
        }
    }
    ...
    public static string PlayerReportFeedback(PlayerControl player)
    {
        ...
        if (player.Is(RoleEnum.Blackmailer) || player.Is(RoleEnum.Detective) || player.Is(RoleEnum.Doomsayer)
        ...
        if (player.Is(RoleEnum.Altruist) || player.Is(RoleEnum.Amnesiac) || player.Is(RoleEnum.Janitor)
        ...
        if (player.Is(RoleEnum.Investigator) || player.Is(RoleEnum.Swooper) || player.Is(RoleEnum.Tracker)
        ...
        if (player.Is(RoleEnum.Arsonist) || player.Is(RoleEnum.Miner) || player.Is(RoleEnum.Plaguebearer)
        ...
        if (player.Is(RoleEnum.Engineer) || player.Is(RoleEnum.Escapist) || player.Is(RoleEnum.Grenadier)
        ...
        if (player.Is(RoleEnum.Executioner) || player.Is(RoleEnum.Jester) || player.Is(RoleEnum.Mayor)
        ...
        if (player.Is(RoleEnum.Bomber) || player.Is(RoleEnum.Juggernaut) || player.Is(RoleEnum.Pestilence)
            || player.Is(RoleEnum.Sheriff) || player.Is(RoleEnum.Vigilante) || player.Is(RoleEnum.Warlock))
            return
                $"You observe that {player.GetDefaultOutfit().PlayerName} is capable of performing relentless attacks";
        if (player.Is(RoleEnum.Crewmate) || player.Is(RoleEnum.Impostor))
