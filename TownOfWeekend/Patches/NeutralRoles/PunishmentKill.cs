...
namespace TownOfWeekend.Patches.NeutralRoles;
...
public class PunishmentKill
{
    ...
    public IEnumerator Open(float delay)
    {
        ...
        Targets = PlayerControl.AllPlayerControls.ToArray()
            .Where(x => Inclusion(x) && !x.Data.IsDead && !x.Data.Disconnected).ToList();
        Logger<TownOfWeekendPlugin>.Warning($"Targets {Targets.Count}");
        if (Menu == null)
        {
            if (Camera.main == null)
                yield break;
    ...
    private static ShapeshifterMinigame GetShapeshifterMenu()
    {
        ...
        return GameObject.Instantiate(rolePrefab?.Cast<ShapeshifterRole>(), GameData.Instance.transform)
            .ShapeshifterMenu;
    }
    ...
    public static class MenuPatch
    {
        public static bool Prefix(ShapeshifterMinigame __instance)
        {
            ...
            for (var i = 0; i < menu.Targets.Count; i++)
            {
                ...
                panel.transform.localPosition = new Vector3(__instance.XStart + num * __instance.XOffset,
                    __instance.YStart + num2 * __instance.YOffset, -1f);
            ...
            ControllerManager.Instance.OpenOverlayMenu(__instance.name, __instance.BackButton,
                __instance.DefaultButtonSelected, list2);
            return false;
        }
    }
