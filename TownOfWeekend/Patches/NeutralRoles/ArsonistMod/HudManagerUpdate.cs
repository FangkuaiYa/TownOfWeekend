...
{
    public static Sprite IgniteSprite => ResourcesManager.IgniteSprite;
    ...
    public static void Postfix(HudManager __instance)
    {
        ...
        role.IgniteButton.gameObject.SetActive(
            (__instance.UseButton.isActiveAndEnabled || __instance.PetButton.isActiveAndEnabled)
            && !MeetingHud.Instance && !PlayerControl.LocalPlayer.Data.IsDead
            && AmongUsClient.Instance.GameState == InnerNetClient.GameStates.Started);
        if (!role.LastKiller || !CustomGameOptions.IgniteCdRemoved)
        ...
        if (role.DousedAlive > 0) Utils.SetTarget(ref role.ClosestPlayerIgnite, role.IgniteButton, float.NaN, doused);
    }
}