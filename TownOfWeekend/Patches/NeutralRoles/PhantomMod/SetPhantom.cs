...
namespace TownOfWeekend.NeutralRoles.PhantomMod;
...
public static class AirshipExileController_WrapUpAndSpawn
{
    ...
    {
        SetPhantom.ExileControllerPostfix(__instance);
    }
}
...
public class SetPhantom
{
    ...
    public static void ExileControllerPostfix(ExileController __instance)
    {
        ...
        if (!WillBePhantom.Data.IsDead &&
            (exiled.Is(Faction.NeutralKilling) || exiled.Is(Faction.NeutralEvil) || exiled.Is(Faction.NeutralBenign)) &&
            !exiled.IsLover()) WillBePhantom = exiled;
        if (exiled == WillBePhantom && exiled.Is(RoleEnum.Jester)) return;
        var doomRole = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Doomsayer && ((Doomsayer)x).WonByGuessing && ((Doomsayer)x).Player == WillBePhantom);
        if (doomRole != null) return;
        var exeRole = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Executioner && ((Executioner)x).TargetVotedOut &&
            ((Executioner)x).Player == WillBePhantom);
        if (exeRole != null) return;
        var jestRole = Role.AllRoles.FirstOrDefault(x =>
            x.RoleType == RoleEnum.Jester && ((Jester)x).VotedOut && ((Jester)x).Player == WillBePhantom);
        ...
        WillBePhantom.gameObject.GetComponent<PassiveButton>().OnClick = new Button.ButtonClickedEvent();
        WillBePhantom.gameObject.GetComponent<PassiveButton>().OnClick
            .AddListener((Action)(() => WillBePhantom.OnClick()));
        ...
        List<Vent> vents = new();
        var CleanVentTasks = PlayerControl.LocalPlayer.myTasks.ToArray()
            .Where(x => x.TaskType == TaskTypes.VentCleaning).ToList();
