...
public class RepickPhantom
{
    private static void Postfix(HudManager __instance)
    {
        ...
        if (!PlayerControl.LocalPlayer.Is(Faction.NeutralKilling) &&
            !PlayerControl.LocalPlayer.Is(Faction.NeutralEvil) && !PlayerControl.LocalPlayer.Is(Faction.NeutralBenign))
        {
            var toChooseFromAlive = PlayerControl.AllPlayerControls.ToArray().Where(x =>
                (x.Is(Faction.NeutralKilling) || x.Is(Faction.NeutralEvil) || x.Is(Faction.NeutralBenign)) &&
                !x.Is(ModifierEnum.Lover) && !x.Data.Disconnected).ToList();
        ...
        var toChooseFrom = PlayerControl.AllPlayerControls.ToArray().Where(x =>
            !x.Is(Faction.Crewmates) && !x.Is(Faction.Impostors) && !x.Is(ModifierEnum.Lover) && x.Data.IsDead &&
