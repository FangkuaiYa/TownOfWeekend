...
public class Trap
{
    ...
    public void Update()
    {
        foreach (var player in PlayerControl.AllPlayerControls)
        {
            ...
            //PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage($"player with byte {player.PlayerId} is {Vector2.Distance(transform.position, player.GetTruePosition())} away");
            if (Vector2.Distance(transform.position, player.GetTruePosition()) <
                (CustomGameOptions.TrapSize + 0.01f) * ShipStatus.Instance.MaxLightRadius)
            {
            ...
            if (players.ContainsKey(entry.PlayerId))
            {
                ...
                //PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage($"player with byte {entry} is logged with time {players[entry]}");
                if (players[entry.PlayerId] > CustomGameOptions.MinAmountOfTimeInTrap)
                    foreach (Trapper t in Role.GetRoles(RoleEnum.Trapper))
                    {
                        ...
                        if (!t.trappedPlayers.Contains(playerrole) && entry != t.Player)
                            t.trappedPlayers.Add(playerrole);
                    }
            }
        }
    }
...
public static class TrapExtentions
{
    ...
    public static Trap CreateTrap(this Vector3 location)
    {
        ...
        TrapPref.name = "Trap";
        TrapPref.transform.localScale = new Vector3(
            CustomGameOptions.TrapSize * ShipStatus.Instance.MaxLightRadius * 2f,
            CustomGameOptions.TrapSize * ShipStatus.Instance.MaxLightRadius * 2f,
            CustomGameOptions.TrapSize * ShipStatus.Instance.MaxLightRadius * 2f);
