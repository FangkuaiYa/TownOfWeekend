...
public class RegisterExtraVotes
{
    ...
    public static class VotingComplete
    {
            ...
            [HarmonyArgument(2)] bool tie)
        {
            ...
            PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage($"Exiled PlayerName = {exiledString}");
            PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage($"Was a tie = {tie}");
        }
    }
                    ...
                    foreach (var pros in Role.GetRoles(RoleEnum.Prosecutor))
                    {
                        ...
                        if (prosRole.ProsecuteThisMeeting)
                            if (voteState.VoterId == prosRole.Player.PlayerId)
                            {
                                ...
                                {
                                    __instance.BloopAVoteIcon(playerInfo, amountOfSkippedVoters,
