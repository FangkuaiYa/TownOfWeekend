    ...
    private static void Postfix(HudManager __instance)
    {
        ...
        if (!PlayerControl.LocalPlayer.Is(Faction.Crewmates))
        {
            var toChooseFromAlive = PlayerControl.AllPlayerControls.ToArray().Where(x =>
                x.Is(Faction.Crewmates) && !x.Is(ModifierEnum.Lover) && !x.Data.Disconnected).ToList();
        ...
        var toChooseFrom = PlayerControl.AllPlayerControls.ToArray().Where(x =>
            x.Is(Faction.Crewmates) && !x.Is(ModifierEnum.Lover) && x.Data.IsDead && !x.Data.Disconnected).ToList();
