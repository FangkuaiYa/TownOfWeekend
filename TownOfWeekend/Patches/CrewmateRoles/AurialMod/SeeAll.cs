...
public class SeeAll
{
    ...
    public static bool Check(Aurial s, PlayerControl p)
    {
        ...
        if (s.knownPlayerRoles.TryGetValue(p.PlayerId, out var count))
            if (count >= CustomGameOptions.RadiateCount)
                return true;
        return false;
    }
    ...
    public static void ColorChar(PlayerControl p, Color c)
    {
        ...
        if ((fit != CustomPlayerOutfitType.Aurial && fit != CustomPlayerOutfitType.Camouflage &&
             fit != CustomPlayerOutfitType.Swooper) || (fit == CustomPlayerOutfitType.Aurial && p.myRend().color != c))
        {
