...
namespace TownOfWeekend.ImpostorRoles.BlackmailerMod;
...
public class BlackmailMeetingUpdate
{
    ...
    {
        public static Sprite Letter => ResourcesManager.BlackmailLetterSprite;
        ...
        public static void Postfix(MeetingHud __instance)
        {
            ...
            foreach (var role in blackmailers)
            {
                ...
                {
                    var playerState =
                        __instance.playerStates.FirstOrDefault(x => x.TargetPlayerId == role.Blackmailed.PlayerId);
    ...
    {
        public static Sprite Overlay => ResourcesManager.BlackmailOverlaySprite;
    ...
    public class StopChatting
    {
        public static bool Prefix(TextBoxTMP __instance)
        {
            var blackmailers = Role.AllRoles.Where(x => x.RoleType == RoleEnum.Blackmailer && x.Player != null)
                .Cast<Blackmailer>();
            foreach (var role in blackmailers)
                if (MeetingHud.Instance && role.Blackmailed != null && !role.Blackmailed.Data.IsDead &&
                    role.Blackmailed.PlayerId == PlayerControl.LocalPlayer.PlayerId)
                    return false;
