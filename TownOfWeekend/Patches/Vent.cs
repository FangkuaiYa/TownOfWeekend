...
public static class HudManagerVentPatch
{
    ...
    public static void Postfix(HudManager __instance)
    {
        ...
        var active = PlayerControl.LocalPlayer != null &&
                     VentPatches.CanVent(PlayerControl.LocalPlayer, PlayerControl.LocalPlayer.CachedPlayerData) &&
                     !MeetingHud.Instance;
...
public static class VentPatches
{
    public static bool CanVent(PlayerControl player, NetworkedPlayerInfo playerInfo)
    {
        ...
        if (CustomGameOptions.GameMode == GameMode.Cultist && player.Is(RoleEnum.Engineer)) return true;
        ...
        if ((player.Is(RoleEnum.Morphling) && !CustomGameOptions.MorphlingVent)
            ...
            || (player.Is(RoleEnum.Undertaker) && Role.GetRole<Undertaker>(player).CurrentlyDragging != null &&
                !CustomGameOptions.UndertakerVentWithBody))
            return false;
        ...
        ref float __result)
    {
        ...
        if (GameOptionsManager.Instance.CurrentGameOptions.GameMode == GameModes.Normal)
            couldUse = CanVent(playerControl, playerInfo) && !playerControl.MustCleanVent(__instance.Id) &&
                 ...
                 playerControl.Data.IsImpostor()) couldUse = false;
        ...
        if (canUse)
        {
            ...
            if (__instance.Id == 14 && SubmergedCompatibility.isSubmerged())
                canUse &= num <= (double)__instance.UsableDistance;
            else
                canUse = ((canUse ? 1 : 0) & (num > (double)__instance.UsableDistance ? 0 :
                    !PhysicsHelpers.AnythingBetween(playerControl.Collider, center, position,
