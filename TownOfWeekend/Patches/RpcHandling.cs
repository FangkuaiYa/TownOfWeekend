...
using TownOfWeekend.CrewmateRoles.ImitatorMod;
using TownOfWeekend.CrewmateRoles.MayorMod;
using TownOfWeekend.CrewmateRoles.MedicMod;
using TownOfWeekend.CrewmateRoles.SwapperMod;
using TownOfWeekend.CrewmateRoles.VigilanteMod;
using TownOfWeekend.CultistRoles.NecromancerMod;
using TownOfWeekend.CustomOption;
using TownOfWeekend.Extensions;
using TownOfWeekend.ImpostorRoles.MinerMod;
using TownOfWeekend.ImpostorRoles.TraitorMod;
using TownOfWeekend.Modifiers.AssassinMod;
using TownOfWeekend.NeutralRoles.DoomsayerMod;
using TownOfWeekend.NeutralRoles.ExecutionerMod;
using TownOfWeekend.NeutralRoles.GuardianAngelMod;
using TownOfWeekend.NeutralRoles.PhantomMod;
using TownOfWeekend.NeutralRoles.VampireMod;
using TownOfWeekend.Patches;
using TownOfWeekend.Patches.NeutralRoles;
using TownOfWeekend.Roles;
using TownOfWeekend.Roles.Cultist;
using TownOfWeekend.Roles.Modifiers;
using UnityEngine;
using Coroutine = TownOfWeekend.ImpostorRoles.JanitorMod.Coroutine;
using Object = UnityEngine.Object;
using PerformKillButton = TownOfWeekend.NeutralRoles.AmnesiacMod.PerformKillButton;
...
public static class RpcHandling
{
    ...
    private static void GenEachRole(List<NetworkedPlayerInfo> infected)
    {
        ...
        {
            var benign = PickRoleCount(CustomGameOptions.MinNeutralBenignRoles,
                Math.Min(CustomGameOptions.MaxNeutralBenignRoles, NeutralBenignRoles.Count));
            var evil = PickRoleCount(CustomGameOptions.MinNeutralEvilRoles,
                Math.Min(CustomGameOptions.MaxNeutralEvilRoles, NeutralEvilRoles.Count));
            var killing = PickRoleCount(CustomGameOptions.MinNeutralKillingRoles,
                Math.Min(CustomGameOptions.MaxNeutralKillingRoles, NeutralKillingRoles.Count));
            ...
            {
                var canSubtractBenign = canSubtract(benign, CustomGameOptions.MinNeutralBenignRoles);
                var canSubtractEvil = canSubtract(evil, CustomGameOptions.MinNeutralEvilRoles);
                var canSubtractKilling = canSubtract(killing, CustomGameOptions.MinNeutralKillingRoles);
            ...
            if (NeutralKillingRoles.Contains((typeof(Vampire), CustomGameOptions.VampireOn, true)) &&
                CustomGameOptions.VampireHunterOn > 0)
                CrewmateRoles.Add((typeof(VampireHunter), CustomGameOptions.VampireHunterOn, true));

            CrewmateRoles.SortRoles(crewmates.Count - NeutralBenignRoles.Count - NeutralEvilRoles.Count -
                                    NeutralKillingRoles.Count);
        ...
        if (CustomGameOptions.GameMode == GameMode.AllAny)
        {
            ...
            if (neutRoles.Count > 0)
            {
                ...
                if (neutRoles[0].Item3) neutRoles.Remove(neutRoles[0]);
            }
            ...
            if (CrewmateRoles.Count > 0)
            {
                ...
                if (CrewmateRoles[0].Item3) CrewmateRoles.Remove(CrewmateRoles[0]);
            }
            else
            {
            ...
            while (crewRoles.Count < crewmates.Count && allAnyRoles.Count > 0)
            {
                ...
                if (allAnyRoles[0].Item3) allAnyRoles.Remove(allAnyRoles[0]);
            }
            ...
            while (impRoles.Count < impostors.Count && ImpostorRoles.Count > 0)
            {
                ...
                if (ImpostorRoles[0].Item3) ImpostorRoles.Remove(ImpostorRoles[0]);
            }
        }
        else
        {
        ...
        foreach (var (type, _, unique) in crewRoles) Role.GenRole<Role>(type, crewmates);
        foreach (var (type, _, unique) in impRoles) Role.GenRole<Role>(type, impostors);
        ...
        var canHaveAbility = PlayerControl.AllPlayerControls.ToArray().Where(player => player.Is(Faction.Impostors))
            .ToList();
        canHaveAbility.Shuffle();
        var canHaveAbility2 = PlayerControl.AllPlayerControls.ToArray()
            .Where(player => player.Is(Faction.NeutralKilling)).ToList();
        canHaveAbility2.Shuffle();

        var assassinConfig = new[]
        {
            (canHaveAbility, CustomGameOptions.NumberOfImpostorAssassins),
            (canHaveAbility2, CustomGameOptions.NumberOfNeutralAssassins)
        };
        foreach (var (abilityList, maxNumber) in assassinConfig)
        {
            var assassinNumber = maxNumber;
        ...
        var canHaveAssassinModifier = PlayerControl.AllPlayerControls.ToArray()
            .Where(player => player.Is(Faction.Impostors) && player.Is(AbilityEnum.Assassin)).ToList();
        ...
        var canHaveImpModifier = PlayerControl.AllPlayerControls.ToArray()
            .Where(player => player.Is(Faction.Impostors) && !player.Is(ModifierEnum.DoubleShot)).ToList();
        ...
        var canHaveModifier = PlayerControl.AllPlayerControls.ToArray()
            .Where(player => !player.Is(ModifierEnum.Disperser) && !player.Is(ModifierEnum.DoubleShot) &&
                             !player.Is(ModifierEnum.Underdog))
            .ToList();
        ...
        var toChooseFromCrew = PlayerControl.AllPlayerControls.ToArray()
            .Where(x => x.Is(Faction.Crewmates) && !x.Is(RoleEnum.Mayor) && !x.Is(ModifierEnum.Lover)).ToList();
        ...
        var toChooseFromNeut = PlayerControl.AllPlayerControls.ToArray().Where(x =>
            (x.Is(Faction.NeutralBenign) || x.Is(Faction.NeutralEvil) || x.Is(Faction.NeutralKilling)) &&
            !x.Is(ModifierEnum.Lover)).ToList();
        ...
        var exeTargets = PlayerControl.AllPlayerControls.ToArray().Where(x =>
            x.Is(Faction.Crewmates) && !x.Is(ModifierEnum.Lover) && !x.Is(RoleEnum.Mayor) && !x.Is(RoleEnum.Swapper) &&
            !x.Is(RoleEnum.Vigilante) && x != SetTraitor.WillBeTraitor).ToList();
        ...
        var goodGATargets = PlayerControl.AllPlayerControls.ToArray()
            .Where(x => x.Is(Faction.Crewmates) && !x.Is(ModifierEnum.Lover)).ToList();
        var evilGATargets = PlayerControl.AllPlayerControls.ToArray().Where(x =>
            (x.Is(Faction.Impostors) || x.Is(Faction.NeutralKilling)) && !x.Is(ModifierEnum.Lover)).ToList();
    ...
    private static void GenEachRoleKilling(List<NetworkedPlayerInfo> infected)
    {
        ...
        foreach (var (type, _, _) in crewAndNeutralRoles) Role.GenRole<Role>(type, crewmates);
        foreach (var (type, _, _) in ImpostorRoles) Role.GenRole<Role>(type, impostors);
    }
    ...
    private static void GenEachRoleCultist(List<NetworkedPlayerInfo> infected)
    {
        ...
        if (CustomGameOptions.MayorCultistOn > 0)
            specialRoles.Add((typeof(Mayor), CustomGameOptions.MayorCultistOn, true));
        if (CustomGameOptions.SeerCultistOn > 0)
            specialRoles.Add((typeof(CultistSeer), CustomGameOptions.SeerCultistOn, true));
        if (CustomGameOptions.SheriffCultistOn > 0)
            specialRoles.Add((typeof(Sheriff), CustomGameOptions.SheriffCultistOn, true));
        if (CustomGameOptions.SurvivorCultistOn > 0)
            specialRoles.Add((typeof(Survivor), CustomGameOptions.SurvivorCultistOn, true));
        if (specialRoles.Count > CustomGameOptions.SpecialRoleCount)
            specialRoles.SortRoles(CustomGameOptions.SpecialRoleCount);
        ...
        foreach (var (type, _, unique) in specialRoles) Role.GenRole<Role>(type, crewmates);
        foreach (var (type, _, unique) in crewRoles) Role.GenRole<Role>(type, crewmates);
        foreach (var (type, _, unique) in impRole) Role.GenRole<Role>(type, impostors);
            ...
            switch ((CustomRPC)callId)
            {
                ...
                case CustomRPC.Start:
                    ...
                    ShowRoundOneShield.FirstRoundShielded =
                        readByte == byte.MaxValue ? null : Utils.PlayerById(readByte);
                ...
                case CustomRPC.SetSwaps:
                    ...
                    PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage("Bytes received - " + readSByte +
                        " - " +
                        readSByte2);
                    break;
                ...
                case CustomRPC.SetUntransportable:
                    if (PlayerControl.LocalPlayer.Is(RoleEnum.Transporter))
                        Role.GetRole<Transporter>(PlayerControl.LocalPlayer).UntransportablePlayers
                            .Add(reader.ReadByte(), DateTime.UtcNow);
                    break;
                ...
                case CustomRPC.AltruistRevive:
                    ...
                    foreach (var body in theDeadBodies)
                        if (body.ParentId == readByte)
                        {
                            ...
                            Coroutines.Start(
                                TownOfWeekend.CrewmateRoles.AltruistMod.Coroutine.AltruistRevive(body,
                                    altruistRole));
                        }
                ...
                case CustomRPC.CatchPhantom:
                    ...
                    if (PlayerControl.LocalPlayer == phantomPlayer)
                ...
                case CustomRPC.PhantomWin:
                    ...
                    if (!CustomGameOptions.NeutralEvilWinEndsGame)
                    {
                        ...
                        if (!PlayerControl.LocalPlayer.Is(RoleEnum.Phantom) || !CustomGameOptions.PhantomSpook ||
                        ...
                        var toKill = MeetingHud.Instance.playerStates
                            ...
                            .Select(x => x.TargetPlayerId).ToArray();
                ...
                case CustomRPC.CatchHaunter:
                    ...
                    if (PlayerControl.LocalPlayer == haunterPlayer)
                        HudManager.Instance.AbilityButton.gameObject.SetActive(true);
                ...
                case CustomRPC.TraitorSpawn:
                    ...
                    var killsList = (oldRole.CorrectKills, oldRole.IncorrectKills, oldRole.CorrectAssassinKills,
                        oldRole.IncorrectAssassinKills);
                ...
                case CustomRPC.SetSettings:
                    ...
                    GameOptionsManager.Instance.currentNormalGameOptions.MapId =
                        readByte == byte.MaxValue ? (byte)0 : readByte;
                    GameOptionsManager.Instance.currentNormalGameOptions.RoleOptions.SetRoleRate(RoleTypes.Scientist, 0,
                        0);
                    GameOptionsManager.Instance.currentNormalGameOptions.RoleOptions.SetRoleRate(RoleTypes.Engineer, 0,
                        0);
                    GameOptionsManager.Instance.currentNormalGameOptions.RoleOptions.SetRoleRate(
                        RoleTypes.GuardianAngel, 0, 0);
                    GameOptionsManager.Instance.currentNormalGameOptions.RoleOptions.SetRoleRate(RoleTypes.Shapeshifter,
                        0, 0);
    ...
    public static class RpcSetRole
    {
        ...
        {
            PluginSingleton<TownOfWeekendPlugin>.Instance.Log.LogMessage("RPC SET ROLE");
