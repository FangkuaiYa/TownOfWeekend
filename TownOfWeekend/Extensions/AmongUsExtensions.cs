...
public static class AmongUsExtensions
{
    ...
    public static VisualAppearance GetAppearance(this PlayerControl player)
    {
        if (player.TryGetAppearance(Role.GetRole(player) as IVisualAlteration, out var appearance))
            return appearance;
        if (player.TryGetAppearance(Modifier.GetModifier(player) as IVisualAlteration, out appearance))
            return appearance;
    ...
    public static void SetOutfit(this PlayerControl playerControl, CustomPlayerOutfitType CustomOutfitType)
    {
        ...
        if (!playerControl.Data.Outfits.ContainsKey(outfitType)) return;
        ...
        if (PlayerControl.LocalPlayer.Data.IsImpostor() && playerControl.Data.IsImpostor())
            playerControl.nameText().color = Colors.Impostor;
    }
    ...
    public static TextMeshPro nameText(this PlayerControl p)
    {
        return p?.cosmetics?.nameText;
    }

    public static TextMeshPro NameText(this PoolablePlayer p)
    {
        return p.cosmetics.nameText;
    }

    public static SpriteRenderer myRend(this PlayerControl p)
    {
        return p.cosmetics.currentBodySprite.BodySprite;
    }
}